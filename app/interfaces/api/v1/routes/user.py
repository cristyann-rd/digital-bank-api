...
from app.interfaces.api.schemas.user.user import UserResponse, UserCreate
from app.interfaces.api.dependencies import get_user_service, get_current_user
...
async def get_user(user_id: int, _: UserResponse = Depends(get_current_user),
...
async def update_user(user_id: int, user_data: UserCreate, _: UserResponse = Depends(get_current_user),
                      user_service: UserService = Depends(get_user_service)):
    user = await user_service.get_by_id(user_id)
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Usuário não encontrado")
